...
private:
    void spawn_turtle()
    {
        ...
        request->x = 5.0;
        request->y = 5.0;
        RCLCPP_INFO(this->get_logger(), "Spawn Turtle");
    ...
    void kill_turtle()
    {
        ...
        request->name = turtle_name_;
        RCLCPP_INFO(this->get_logger(), "Remove Turtle");
        auto result = kill_turtle_client_->async_send_request(request);
        RCLCPP_INFO(this->get_logger(), "Removed the turtle");
